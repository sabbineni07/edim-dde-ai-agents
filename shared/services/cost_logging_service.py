...
class CostLoggingService:
    ...
    def _log_to_app_insights(self, request_id, model_name, chain_name, input_tokens, output_tokens, total_tokens, cost_usd, job_id=None):
        logger.info(
            "token_usage",
    ...
    def get_daily_summary(self, date: date) -> Optional[Dict]:
        ...
        try:
            session = get_database_session()
            try:
                summary = session.query(DailyCostSummary).filter(DailyCostSummary.date == date).first()
                if summary:
                    return {
