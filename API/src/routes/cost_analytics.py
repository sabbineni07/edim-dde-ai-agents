...
async def get_daily_cost_summary(
    target_date: Optional[date] = Query(None, description="Date to get summary for (YYYY-MM-DD). Defaults to today.")
):
    ...
    try:
        ...
        summary = cost_logger.get_daily_summary(target_date)
...
async def get_job_cost_breakdown(
    ...
    """
    try:
        logs = cost_logger.get_cost_by_job(job_id, days)
    ...
    try:
        ...
        try:
            ...
            results = session.query(
                ...
                CostUsageLog.timestamp < datetime.combine(end_date + timedelta(days=1), datetime.min.time())
            ).first()
